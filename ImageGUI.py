...
class ImageGUI:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.original_image: Optional[Image.Image] = None  # The original image selected
        self.processed_image: Optional[Image.Image] = None  # The image after processing

        # GUI elements
        self.image_label: tk.Label = tk.Label(self.root)
        self.load_button: tk.Button = tk.Button(self.root,
                                                text="Select a picture directory...",
                                                command=self.load_images)
        self.prev_button: tk.Button = tk.Button(self.root,
                                                text="Previous",
                                                command=self.previous_image)
        self.next_button: tk.Button = tk.Button(self.root,
                                                text="Next",
                                                command=self.next_image)
        self.select_button: tk.Button = tk.Button(self.root,
                                                  text="Select this",
                                                  command=self.select_image)
        self.rotate_button: tk.Button = tk.Button(self.root,
                                                  text="Rotate",
                                                  command=self.rotate_image)
        self.grayscale_button: tk.Button = tk.Button(self.root,
                                                     text="Grayscale",
                                                     command=self.convert_to_grayscale)
        self.show_original_button: tk.Button = tk.Button(self.root,
                                                         text="Show Original Image",
                                                         command=self.show_original_image)
        self.save_button: tk.Button = tk.Button(self.root,
                                                text="Save Image",
    ...
    def setup_initial_gui(self) -> None:
    ...
    def setup_processing_gui(self) -> None:
    ...
    def display_image_in_label(self, image: Optional[Image.Image]) -> None:
        """Display the given PIL Image in the image label."""
        if image:
            img_resized: Image.Image = image.resize((500, 500))
            img_tk: ImageTk.PhotoImage = ImageTk.PhotoImage(img_resized)
            self.image_label.config(image=img_tk)
            self.image_label.image = img_tk  # Keep a reference

    def show_image(self, image_path: str) -> None:
        """Load and display the image from the given path."""
        image: Optional[Image.Image] = self.load_image(image_path)
        if image:
